...
public class  Updatelibrary extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
       ...
       try{
          ...
          String sql="UPDATE library SET bookname=?,authorname=?,category=? WHERE bookid=?";
          PreparedStatement pst=conn.prepareStatement(sql);
           pst.setString(1, mybookname);
           pst.setString(2,myauthorname);
           pst.setString(3,mycategory);
           pst.setInt(4,Integer.parseInt(mybookid));
           int count=pst.executeUpdate();
           if(count>0)
           {
               response.setContentType("text/html");
               out.print("<h3>success</h3>");
              RequestDispatcher rd=request.getRequestDispatcher("/Updatelibrary.jsp");
